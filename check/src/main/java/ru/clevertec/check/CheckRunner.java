...
public class CheckRunner {
    ...
    public static void main(String[] args) {
                ...
                if (product != null) {
                    if (requestedQuantity <= product.getQuantity()) {
                        ...
                        if (product.isWholesaleProduct() && requestedQuantity >= 5) {
                            discountStrategy = new WholesaleDiscountStrategy();
                        } else {
        ...
        checkBuilder.addTotal(new BigDecimal(formattedTotalSum), new BigDecimal(formattedTotalDiscountAmount), new BigDecimal(formattedTotalWithDiscount));

        System.out.println("Finalizing order details...");

        writeToResultCSV(checkBuilder.build());
...
class DiscountCard {
    ...
    @Override
    public String toString() {
        return String.format("{cardNumber='%s', discountPercentage=%d}", cardNumber, discountPercentage);
    }
}
